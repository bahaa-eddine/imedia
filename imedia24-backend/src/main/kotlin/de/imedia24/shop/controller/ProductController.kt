...
class ProductController(private val productService: ProductService) {
    ...
    @PatchMapping("/products/{sku}", produces = ["application/json"])
    fun partiallyUpdateProduct(@PathVariable("sku") sku: String, @RequestBody product: UpdateProductDTO) : ResponseEntity<ProductResponse> {
        logger.info("Request to partially update product: {}", sku)
        val updatedProduct = productService.partiallyUpdateProduct(sku, product)
        return if (updatedProduct != null) {
            ResponseEntity.ok().body(updatedProduct)
        } else {
            ResponseEntity.notFound().build()
    ...
    @DeleteMapping("/products/{sku}", produces = ["application/json"])
    fun deleteProduct(@PathVariable("sku") sku: String): ResponseEntity<Void> {
        logger.info("Request to delete product: {}", sku)
        return if (productService.deleteProduct(sku)) {
